...
def get_submenu(main_menu_string, list_of_data):

    max_item_length = len(main_menu_string.splitlines()[0]) - 4
    # Get the length of the topper string (the equals along the top)
    topper_length = len(main_menu_string.splitlines()[0])
...
quizes = [
    "What is 1+1?",
    "Do you like cheese?",
    "Have you ever played the hit game?"
]

max_item_length = 30

menu_string = get_menu(menu_data, max_item_length)
submenu_string = get_submenu(menu_string, quizes)

print(menu_string)
print(submenu_string)
